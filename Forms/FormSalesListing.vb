...
#Region "Event Handlers"
    ...
    Private Sub grdList_DoubleClickRow(sender As Object, e As DoubleClickRowEventArgs) Handles grdList.DoubleClickRow
        Try
            If e.Row.Band.Index = 0 Then
                e.Row.Expanded = Not e.Row.Expanded
            End If
        Catch ex As Exception
            ex.ShowError()
        End Try
    ...
    Private Sub txtSearch_ValueChanged(sender As Object, e As EventArgs) Handles txtSearch.ValueChanged
        Try
            Dim searchKeys As String = "sale_code,customer,cashier"
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        Try
            Using dlg As New SaveFileDialog()
                ...
                dlg.DefaultExt = "xls"
                dlg.Title = "Export Listing to Excel"
                dlg.CheckPathExists = True
                dlg.OverwritePrompt = True

                If dlg.ShowDialog(Me) = DialogResult.OK Then
                    If Not IsNothing(gridExporter.Export(grdList, dlg.FileName)) Then
                        ShowMessage("Sales Listing exported successfully!")
                    End If
                End If
            End Using
        Catch ex As Exception
...
#Region "Methods"
    ...
    Private Sub RefreshList()
        Try
            With New SalesList
                grdList.DataSource = .GetList(dateRange.StartDate, dateRange.EndDate)
                txtSearch.Clear()
            End With
